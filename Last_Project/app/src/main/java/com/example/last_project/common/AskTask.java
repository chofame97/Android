...
public class AskTask extends AsyncTask<String , String , InputStream> {
    ...
    protected InputStream doInBackground(String... strings) {
        postUrl = CommonVal.HTTP_IP + CommonVal.SVRPATH + mapping ; //http://192.168.0.47/mid/
        ...
        for(int i = 0 ; i < fileList.size() ; i++){
            builder.addPart(fileList.get(i).getKey() ,
                    new FileBody(new File(fileList.get(i).getValue()))
                    );
        } // EOF <- 오류가 나면 해당하는 파일을 버퍼형태의 바이트 형식으로 못바꾸는 파일
