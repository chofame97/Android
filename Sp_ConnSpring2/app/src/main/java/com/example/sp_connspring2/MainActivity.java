    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.test).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // 메인액티비티는 액티비티 자체 작업을 따로하고 AsnckTask도 작업 따로
                // execute().get() 이 싱크 테스크를 실행하고나서 반드시 결과를 받고
                // 그다음줄 소스를 진행한다

                try {
                   InputStream in = task.execute().get(5000, TimeUnit.MILLISECONDS); // 해당하는 어싱크가 반드시 결과를 return 해줄때 (스프링에서 응답줬을때)
                                            // 다음 스택의 코드를 실행하겠다
                                            // TimeOut 을 주는이유는 서버가 특정한 상황에서 응답을 주지 못할때
                    ...
                    Log.d("dd", "onClick: "+revDTO.getId()+revDTO.getPw());
                } catch (ExecutionException e) {
                    e.printStackTrace();
                } catch (InterruptedException | TimeoutException e) {
                    e.printStackTrace();
                }
                Log.d("ddd", "onClick: 실행됨 ");
            }
        });
    }
}