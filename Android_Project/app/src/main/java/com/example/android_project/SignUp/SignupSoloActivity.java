...
public class SignupSoloActivity extends AppCompatActivity {
    Button btn_join;
    EditText edt_id, edt_pw, edt_pw_chk, edt_name, edt_birth, edt_email, edt_phn, edt_addr;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_sign_up_solo);
        ...
        btn_join.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (edt_id.getText().toString().length() == 0) {
                    Toast.makeText(SignupSoloActivity.this, "아이디를 입력하세요", Toast.LENGTH_SHORT).show();
                    edt_id.requestFocus();
                    return;
                }
                if(!Pattern.matches("^[^0-9]*[a-zA-Z][0-9]{4,15}$",edt_id.getText().toString())){
                    Toast.makeText(SignupSoloActivity.this, "4~10자 영어대소문자 또는 숫자", Toast.LENGTH_SHORT).show();
                    edt_id.requestFocus();
                    return;
                }
                ...
                if (edt_pw.getText().toString().length() == 0) {
                    Toast.makeText(SignupSoloActivity.this, "비밀번호를 입력하세요", Toast.LENGTH_SHORT).show();
                    edt_pw.requestFocus();
                    return;
                }
                if(!Pattern.matches("^.*(?=^.{8,15}$)(?=.*\\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&+=]).*$",edt_pw.getText().toString())){
                    Toast.makeText(SignupSoloActivity.this, "8~20글자 비밀번호", Toast.LENGTH_SHORT).show();
                    edt_pw.requestFocus();
                    return;
                }
                // 비밀번호 확인 유효성
                if ( !edt_pw.getText().toString().equals(edt_pw_chk.getText().toString()) ) {
                    Toast.makeText(SignupSoloActivity.this, "비밀번호가 일치하지 않습니다", Toast.LENGTH_SHORT).show();
                    edt_pw.setText("");
                    edt_pw_chk.setText("");
                    edt_pw.requestFocus();
                    return;
                ...
                if (!Pattern.matches("^\\w{4,11}@[a-z]{2,10}[\\.][a-z]{2,8}[\\.]?[a-z]{0,2}$",edt_email.getText().toString())) {
                    Toast.makeText(SignupSoloActivity.this, "이메일 양식이 틀림", Toast.LENGTH_SHORT).show();
                ...
                if (!Pattern.matches("^[0-9]{11}$",edt_phn.getText().toString())) {
                    Toast.makeText(SignupSoloActivity.this, "전화번호 양식이 틀림", Toast.LENGTH_SHORT).show();
                    edt_phn.requestFocus();
                    return;
                }
                ...
                if (!Pattern.matches("^[가-힣]{2,4}$",edt_name.getText().toString())) {
                    Toast.makeText(SignupSoloActivity.this, "이름 양식이 틀림", Toast.LENGTH_SHORT).show();
                    edt_name.requestFocus();
                    return;
                }

                Intent intent = new Intent(SignupSoloActivity.this, LoginSelectActivity.class);
